...
发现这时编译可以通过了（注意：Java语言中true和1没有关联，二者属于不同的类型，不能相互转换，不存在C语言中整数值非零即真的情况）。<font color=blue>两个不同类型的返回值的加入，使得方法的重载成功了。这是为什么呢？我们知道，Java代码中的方法特征签名只包括了方法名称、参数顺序和参数类型，并不包括方法的返回值，因此方法的返回值并不参与重载方法的选择，这样看来为重载方法加入返回值貌似是错误行不通的。对于重载方法的选择来说，这确实是错误的，但我们现在要解决的问题是让上述代码能通过编译，让两个重载方法能够合理地共存于同一个Class文件之中，这就要看字节码的方法特征签名，它不仅包括了Java代码中方法特征签名中所包含的那些信息，还包括方法返回值及受查异常表。为两个重载方法加入不同的返回值后，因为有了不同的字节码特征签名，它们便可以共存于一个Class文件之中。</font><font color=red>但是需要注意的是，严格来说，针对上述的两个方法，方法名一样，方法参数一样，而方法参数的泛型参数不一样，方法的返回参数不一样的这种情况，并不能算作是Java函数方法的重载，Java中的泛型其实只能算是“伪泛型”，只是在对象进入容器方法时，由编译器进行类型安全检查，确保对象的类型正确，然后在对象离开容器方法时，编译器会自动插入类型转换相关的字节码指令，免去程序员自己手写强制类型转换的代码，在这整个进入到离开之间，对象都是处于相应的原始类型，而非泛型的类型参数所指定的类型。所以在JDK1.6之后的版本中，上述两个method的“重载”方式是错误的，是无法通过编译器编译的，仅仅只能在JDK1.5和JDK1.6的版本上编译通过和运行。</font>
